...
export function SplitBillScreen({ onBack }: { onBack: () => void }) {
  ...
  const [assignments, setAssignments] = useState<Record<string, string>>({}) // itemId -> personId
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      ...
      <div className="flex-1 overflow-y-auto relative">
        ...
        {/* State: mode_select */}
        {view === 'mode_select' && (
          <div className="p-6 flex flex-col h-full animate-in slide-in-from-right-4 duration-300 pb-20 justify-center">
        ...
        {view === 'host_assign' && (
          <div className="p-4 flex flex-col h-full animate-in slide-in-from-right-4 duration-300">
            <div className="flex gap-2 overflow-x-auto pb-3 mb-2 snap-x [&::-webkit-scrollbar]:hidden">
              {people.map(p => (
            ...
            <div className="flex-1 overflow-y-auto mb-20">
              <h2 className="text-sm font-bold text-slate-500 mb-3 px-1">點擊品項分配給成員</h2>
                  ...
                  return (
                    <div key={item.id}>
                      <div 
                        ...
                        className={`w-full bg-white p-4 rounded-xl border-2 flex items-center justify-between cursor-pointer transition-all shadow-sm ${assignments[item.id] ? 'border-green-500' : selectedItemForAssign === item.id ? 'border-indigo-400 ring-2 ring-indigo-100' : 'border-slate-200 hover:border-slate-300'}`}
                      >
                        <div>
            ...
            <div className="absolute bottom-0 left-0 w-full p-4 border-t bg-white shadow-[0_-10px_20px_rgba(0,0,0,0.05)] z-20">
              {!allAssigned && (
                 <div className="bg-amber-50 text-amber-700 text-xs font-bold p-2 mb-3 rounded-lg flex items-center justify-center border border-amber-200 animate-pulse">
        ...
        {view === 'confirm' && (
          <div className="p-6 flex flex-col h-full animate-in slide-in-from-right-4 duration-300 pb-20">
            <h2 className="text-xl font-black text-slate-800 mb-6">確認分帳明細</h2>
            <div className="space-y-4 mb-8">
              {people.map(p => {
                const pItems = MOCK_ITEMS.filter(item => assignments[item.id] === p.id)
                const total = pItems.reduce((sum, item) => sum + item.price, 0)
                ...
                return (
                  <div key={p.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="font-black text-indigo-600 text-xl">${total}</span>
                    </div>
                    <div className="text-sm text-slate-500 pl-10">
                      {pItems.map(item => item.name).join('、')}
                    </div>
                  </div>
                )
              })}
        ...
        {view === 'host_pick_shared' && (
          <div className="p-4 flex flex-col h-full animate-in slide-in-from-right-4 duration-300">
            <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 mb-6 flex items-start shadow-sm">
        ...
        {view === 'share_link' && (
          <div className="p-6 flex flex-col items-center justify-center h-full animate-in zoom-in-95 duration-300 pb-20">
        ...
        {view === 'claim_room' && (
          <div className="p-4 flex flex-col h-full animate-in fade-in duration-300">
                  ...
                  >
                    ...
                    <div>
                      {isMine ? (
                        ...
                        <Badge variant="outline" className="text-slate-400">小智 已領</Badge> 
                      ) : (
            ...
            <div className="absolute bottom-0 left-0 w-full p-4 border-t bg-white shadow-[0_-10px_20px_rgba(0,0,0,0.05)] z-20">
              {!flowBAllClaimed && (
                 <div className="bg-amber-50 text-amber-700 text-xs font-bold p-2 mb-3 rounded-lg flex items-center justify-center border border-amber-200 animate-pulse">
                   <AlertTriangle className="h-4 w-4 mr-1.5" /> 還有 {unassignedFlowBItems.length} 個品項未被認領！
                 </div>
              )}
              <Button onClick={() => setView('payment')} disabled={!flowBAllClaimed} className={`w-full font-bold py-6 rounded-2xl shadow-lg active:scale-95 transition-all text-lg ${flowBAllClaimed ? 'bg-indigo-500 hover:bg-indigo-600 text-white' : 'bg-slate-200 text-slate-400'}`}>
                確認結算我的帳單
              </Button>
            </div>
          </div>
          ...
          <div className="p-6 flex flex-col h-full animate-in slide-in-from-right-4 duration-300 pb-20">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden mb-6">
              <div className="bg-indigo-50 p-4 border-b border-indigo-100 flex justify-between items-center">
                ...
                <span className="text-3xl font-black text-indigo-600">$452</span>
              </div>
              <div className="p-5 space-y-4">
                <div>
                  <h4 className="text-xs font-bold text-slate-400 mb-2">我的個人餐點</h4>
                  <div className="flex justify-between text-sm font-bold text-slate-700"><span>青醬海鮮燉飯</span><span>$220</span></div>
                  <div className="flex justify-between text-sm font-bold text-slate-700 mt-1"><span>松露薯條</span><span>$120</span></div>
                </div>
                <div className="h-px bg-slate-100 w-full"></div>
                <div>
                  <h4 className="text-xs font-bold text-slate-400 mb-2 flex items-center justify-between">公費平分項目 (3人平分) <span className="text-slate-500">+$112</span></h4>
                  <div className="flex justify-between text-sm text-slate-500"><span>10% 服務費 ($66)</span><span>+$22</span></div>
                  <div className="flex justify-between text-sm text-slate-500 mt-1"><span>歡樂拼盤 ($270)</span><span>+$90</span></div>
                </div>
              </div>
            </div>

            <div className="bg-orange-50 rounded-2xl p-4 border border-orange-200 mb-8 flex items-center shadow-sm">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-orange-700">可用 150 點，折抵後只需付 $302</p>
              </div>
